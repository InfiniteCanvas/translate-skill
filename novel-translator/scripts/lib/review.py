...
def write_report(
    ...
    instead of grouped by severity: `## Machine-applicable (apply with
    `review fix`)` holds every finding whose fix is fully determined by
    its structured fields (see command_for_finding), each with a
    `- Command:` bullet after the `- Action:` line so `review fix
    --glossary <this file>` can apply them offline; `## Needs manual
    review (decide yourself or hand to an agent)` holds the rest
    (model-tier duplicate / variant findings, suggestion-less field
    kinds, `other`) with no Command line. Either section is omitted when
    empty; within a section warns precede infos, source-ascending, and
    `[N]` indices number all open findings continuously in reading order
    (machine first, then manual). Findings whose kind-field was fixed
    this run (same (source, field) as an applied fix) are excluded from
    the numbering and listed under "Fixed automatically" instead. The
    header bullet that records the generating command is named
    `- Generated by:` so `- Command:` unambiguously means a fix command.
    ...
    manual: list[dict] = []
    for f in ordered:
        if command_for_finding(f) is None:
            manual.append(f)
        else:
            machine.append(f)
    ...
    lines: list[str] = [
        ...
        f"  warn: {n_warn}",
        f"  info: {n_info}",
        f"machine_applicable: {n_machine}",
        f"manual_review: {n_manual}",
        f"manual_review_indices: [{', '.join(str(i) for i in manual_indices)}]",
        "---",
        "",
    ...
    if not open_findings:
        ...
        idx = 0
        for title, group in (
            ("Machine-applicable (apply with `review fix`)", machine),
            ("Needs manual review (decide yourself or hand to an agent)", manual),
        ):
            if not group:
                continue
            lines += [f"## {title}", ""]
