...
```jsonc
{
  ...
  "contextual_glossary_cap": 80, // max glossary terms injected into one translation prompt
...
primary text on a generated cover.
...
|---|---|---|
| `translation.md` | TRANSLATE (per chunk) | `source_lang target_lang glossary feedback_section chapter_title chunk_info source_lines line_count` |
| `glossary_expand.md` | GLOSSARY_EXPAND | `source_lang target_lang glossary source_lines translation_lines max_terms` |
| `faithfulness.md` | FAITH | `source_lang target_lang source_lines translation_lines` |
| `tn_generate.md` | TN_GENERATE | `source_lang target_lang source_lines translation_lines max_notes` |
...
`ensure_ascii=False`). The `glossary` block renders one line per term as
`source = "translation" (category) — definition` — quoting plus parentheses
keep models from copying the metadata into translations.
...
- TN_GENERATE → `{"notes": [{"line": int, "term": str, "note": str}]}`
