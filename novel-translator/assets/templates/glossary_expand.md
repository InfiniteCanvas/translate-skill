...
Below, the source lines (JSON array of strings, {{source_lang}}) and the translation lines (JSON array of strings, {{target_lang}}) are the same chapter, line for line. Compare them and propose NEW glossary terms that are named entities or named actions: personal names and their aliases, place names, organization/sect names, named artifacts, technique & skill names, named cultivation realms or power states, and titles bound to a name ("Empress Dowager Zhao", "Steward Li").
...
- Named entities are ALWAYS drift-prone: propose every distinct personal name, nickname, or alias; place name; organization/sect name; title bound to a name (e.g. "Steward Li", "Empress Dowager Zhao"); named artifact; named technique or skill; and named realm or power state that appears in this chapter and is not already in the glossary — even if its rendering seems obvious or it appears only once (it may recur in later chapters with a different rendering).
- A title qualifies ONLY when bound to a name: "Empress Dowager Zhao" qualifies, "Empress Dowager" alone does not. What one character calls another — kinship terms and other ways of address such as "great grandmother", "big brother", "my lord" — NEVER qualifies, even when it refers to one specific person: which address is used depends on the speaker and the scene, so it has no single fixed rendering.
...
- An empty list is only correct when the chapter contains NO named entity or named action beyond those already in the glossary.
